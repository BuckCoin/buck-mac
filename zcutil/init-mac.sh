...
if [ ! -f "$HOME/Library/Application Support/Buck/buck.conf" ]; then
    echo "Creating buck.conf"
    mkdir -p "$HOME/Library/Application Support/Buck/"
    echo "rpcuser=zcashrpc" > ~/Library/Application\ Support/Buck/buck.conf
    PASSWORD=$(cat /dev/urandom | env LC_CTYPE=C tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1)
    echo "rpcpassword=$PASSWORD" >> "$HOME/Library/Application Support/Buck/buck.conf"
    echo "Complete!"
fi
